...
execute_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                  Statement *statement);

static StatementResult
execute_expression_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                             Statement *statement)
{
...
static StatementResult
execute_global_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                         Statement *statement)
{
    IdentifierList *pos;
    StatementResult result;
...
static StatementResult
execute_elsif(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
              Elsif *elsif_list, SIMCAR_Boolean *executed)
{
    StatementResult result;
    SIMCAR_Value   cond;
    Elsif *pos;

    *executed = SIMCAR_FALSE;
    result.type = NORMAL_STATEMENT_RESULT;
    for (pos = elsif_list; pos; pos = pos->next) {
        cond = crb_eval_expression(inter, env, pos->condition);
        if (cond.type != SIMCAR_BOOLEAN_VALUE) {
...
static StatementResult
execute_if_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                     Statement *statement)
{
    StatementResult result;
    SIMCAR_Value   cond;
    ...
    DBG_assert(cond.type == SIMCAR_BOOLEAN_VALUE, ("cond.type..%d", cond.type));
...
static StatementResult
execute_while_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                        Statement *statement)
{
    StatementResult result;
    SIMCAR_Value   cond;
    ...
    for (;;) {
        ...
        DBG_assert(cond.type == SIMCAR_BOOLEAN_VALUE,
...
static StatementResult
execute_for_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                      Statement *statement)
{
    StatementResult result;
    SIMCAR_Value   cond;
...
static StatementResult
execute_return_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                         Statement *statement)
{
    StatementResult result;
    ...
    if (statement->u.return_s.return_value) {
        ...
        result.u.return_value.type = SIMCAR_NULL_VALUE;
    }
...
static StatementResult
execute_break_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                        Statement *statement)
{
    StatementResult result;
...
static StatementResult
execute_continue_statement(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                           Statement *statement)
{
    StatementResult result;
...
StatementResult
crb_execute_statement_list(SIMCAR_Interpreter *inter, SIMCAR_LocalEnvironment *env,
                           StatementList *list)
{
    StatementList *pos;
    StatementResult result;

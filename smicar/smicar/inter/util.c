...
static SIMCAR_Interpreter *st_current_interpreter;
...
void
crb_set_current_interpreter(SIMCAR_Interpreter *inter)
{
...
void *
crb_execute_malloc(SIMCAR_Interpreter *inter, size_t size)
{
    void *p;
...
Variable *
crb_search_local_variable(SIMCAR_LocalEnvironment *env, char *identifier)
{
    Variable    *pos;
...
Variable *
crb_add_local_variable(SIMCAR_LocalEnvironment *env, char *identifier)
{
...
Variable *
crb_add_global_variable(SIMCAR_Interpreter *inter, char *identifier)
{
    Variable    *new_variable;
...
void
SIMCAR_add_global_variable(SIMCAR_Interpreter *inter, char *identifier,
                        SIMCAR_Value *value)
{
    Variable    *new_variable;
...
char *
SIMCAR_value_to_string(SIMCAR_Value *value)
{
    VString     vstr;
    ...
    case SIMCAR_ARRAY_VALUE:
        ...
        for (i = 0; i < value->u.object->u.array.size; i++) {
            ...
            new_str = SIMCAR_value_to_string(&value->u.object->u.array.array[i]);

                    ...
                    Block *block)
{
    ...
    f = (FunctionDefinition *)crb_malloc(sizeof(FunctionDefinition));
    f->name = identifier;
...
crb_create_parameter(char *identifier)
{
    ...
    p = (ParameterList *)crb_malloc(sizeof(ParameterList));
    p->name = identifier;
    p->next = NULL;
...
crb_create_argument_list(Expression *expression)
{
    ...
    al = (ArgumentList *)crb_malloc(sizeof(ArgumentList));
...
crb_create_expression_list(Expression *expression)
{
    ...
    el = (ExpressionList *)crb_malloc(sizeof(ExpressionList));
...
crb_create_statement_list(Statement *statement)
{
    ...
    sl = (StatementList *)crb_malloc(sizeof(StatementList));
...
crb_alloc_expression(ExpressionType type)
{
    ...
    exp = (Expression *)crb_malloc(sizeof(Expression));
    exp->type = type;
...
static Expression
convert_value_to_expression(SIMCAR_Value *v)
{
    Expression  expr;

    if (v->type == SIMCAR_INT_VALUE) {
        expr.type = INT_EXPRESSION;
        expr.u.int_value = v->u.int_value;
    } else if (v->type == SIMCAR_DOUBLE_VALUE) {
        expr.type = DOUBLE_EXPRESSION;
        expr.u.double_value = v->u.double_value;
    } else {
        DBG_assert(v->type == SIMCAR_BOOLEAN_VALUE,
                   ("v->type..%d\n", v->type));
...
Expression *
crb_create_binary_expression(ExpressionType operato,
                             Expression *left, Expression *right)
{
    if ((left->type == INT_EXPRESSION
        ...
        *left = convert_value_to_expression(&v);
        ...
        Expression *exp;
        exp = crb_alloc_expression(operato);
...
Expression *
crb_create_boolean_expression(SIMCAR_Boolean value)
{
    Expression *exp;
...
alloc_statement(StatementType type)
{
    ...
    st = (Statement *)crb_malloc(sizeof(Statement));
    st->type = type;
...
crb_create_global_identifier(char *identifier)
{
    ...
    i_list = (IdentifierList *)crb_malloc(sizeof(IdentifierList));
